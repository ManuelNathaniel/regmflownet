...
class ResidualBlock(nn.Module):
    def __init__(self, in_planes, planes, norm_fn='group', stride=1):
        ...
        # In DROFlowNet: self.downsample=nn.Sequential(nn,Conv2d(in_planes,planes, kernel_size=1,stride))
        if stride == 1:
...
class BasicEncoder(nn.Module):
    def __init__(self, output_dim=128, norm_fn='batch', dropout=0.0):
        ...
        # DROFlow中Conv2d(1, 64, kernel_size=7, stride=1, padding=3)
        # 对于粒子图像测速,输入是灰度图，不是RGB图，此处input_channel=2
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3)
        self.relu1 = nn.ReLU(inplace=True)

        self.in_planes = 64
        # DROFlow中stride=2, 2, 2
...
class LayerEncoderBlock(nn.Module):
    ...
    def __init__(self, win_size, d_model, num_heads, d_ff, dropout=0.):
        ...
        self.stride_list = [math.floor(win_size / self.down_factor ** idx) for idx in range(8) if
                            win_size / self.down_factor ** idx >= 1]
        # [16, 4, 1]
    ...
    def cross_attention(self, query, keyVal):
        ...
        keyVal = F.unfold(keyVal_pad.permute(0, 3, 1, 2), self.win_size,
                          stride=self.unfold_stride)  # (N, C*win_size*win_size, num_win)
        keyVal = keyVal.permute(0, 2, 1).reshape(-1, C, self.win_size * self.win_size).permute(0, 2,
                                                                                               1)  # (num_win*B, win_size*win_size, C)
